...
bool confirm(const Options &o, const std::string &prompt) {
  ...
  /* Never block on a prompt we can't actually ask. If stdin isn't a terminal
   * (piped, redirected, run from a service/shim) we'd otherwise hang forever
   * waiting on getchar() -- the `stratum remove --yes` hang came from exactly
   * this. Default to the safe answer (no) and tell the user how to proceed. */
  if (!isatty(STDIN_FILENO)) {
    fprintf(stderr, "%s [y/N] (non-interactive: assuming no; pass --yes to confirm)\n",
            prompt.c_str());
    return false;
  }
  fprintf(stderr, "%s [y/N] ", prompt.c_str());
  int c = getchar();
  /* EOF (Ctrl-D / closed stdin) must also mean "no", not loop or block. */
  if (c == EOF) {
    fprintf(stderr, "\n");
    return false;
  }
...
int cli_main(int argc, char **argv) {
  ...
  std::string cmd;
  /* Global flags (--root/--repo/--key/--yes/--expose/...) are accepted ANYWHERE
   * on the command line, not just before the subcommand. This kills a whole
   * class of "it hung / it ignored my flag" confusion -- e.g. `salt stratum
   * remove arch --yes` used to silently drop the trailing --yes and then block
   * on confirm() forever. Before the command is known, an unrecognized flag is
   * an error (typo guard). After it, an unrecognized flag is passed through
   * untouched so it can reach the inner command (e.g. `salt alpine/nvim -u`).
   * -h/--help and -V/--version are honored only before the subcommand so they
   * don't shadow a stratum command's own flags. */
  for (int i = 1; i < argc; i++) {
    std::string a = argv[i];
    bool pre_cmd = cmd.empty();
    if (a == "--root" && i + 1 < argc) {
      o.root = argv[++i];
      ...
      o.expose_mode = "never";
    } else if (pre_cmd && (a == "-h" || a == "--help")) {
      usage();
      return 0;
    } else if (pre_cmd && (a == "--version" || a == "-V")) {
      printf("salt 0.1.0 (%s)\n", arch_detect().c_str());
      return 0;
    } else if (pre_cmd && !a.empty() && a[0] == '-') {
      fprintf(stderr, "salt: unknown flag '%s'\n", a.c_str());
      return 2;
    } else if (pre_cmd) {
      cmd = a;
    } else {
      /* After the command: any token we didn't recognize as a global flag --
       * including unknown flags -- belongs to the subcommand. */
      rest.push_back(a);
    }
  }
  if (cmd.empty()) {
