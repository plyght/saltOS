...
static void salt_run_bind_file(const char *src, const char *dst_in_root) {
   ...
   * file with a fresh empty file before binding the host's copy over it. Use
   * lstat (not salt_path_exists, which follows the link) so we catch dangling
  ...
  if (mount(src, dst, NULL, MS_BIND, NULL) != 0) {
    /* resolv.conf/hosts/localtime are DNS/clock-critical; a silent failure here
     * is what broke name resolution before. Warn so it's diagnosable instead of
     * failing mysteriously deep inside the stratum command. */
    fprintf(stderr, "salt run: warning: could not bind %s into stratum: %s\n",
            dst_in_root, strerror(errno));
  }
  free(dst);
}
