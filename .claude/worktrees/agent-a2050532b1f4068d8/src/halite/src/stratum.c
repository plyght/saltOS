...
static void write_resolv_conf(const char *target) {
   ...
   * that dangles inside a fresh stratum; copying onto it fails silently and
   * leaves the stratum with no working DNS. Drop anything that isn't a regular
   * file first so we leave a real resolv.conf, and surface a warning if the copy
   * still fails instead of hiding it. */
  struct stat dstat;
  if (lstat(dst, &dstat) == 0 && !S_ISREG(dstat.st_mode)) unlink(dst);
  if (salt_copy_file("/etc/resolv.conf", dst) != SALT_OK)
    fprintf(stderr, "salt: warning: could not seed %s; stratum DNS may not work\n", dst);
  free(dst);
}
