#!/bin/bash
# Build a persistent aarch64 saltOS disk image tuned for Apple's Virtualization
# framework (the "Apple Virtualization" backend in UTM) on Apple Silicon Macs.
#
# Output is a raw GPT disk image (ESP + btrfs root) that boots via the EFI
# bootloader Apple's Virtualization.framework provides (VZEFIBootLoader). Unlike
# the live ISO this is a real read-write install, which is much smoother for
# day-to-day use inside a VM.
#
# Why a separate target instead of the live ISO:
#   * Console:   Apple's framework exposes its serial port as a virtio console
#                (/dev/hvc0), NOT ttyS0/ttyAMA0. The kernel cmdline and the login
#                getty must target hvc0 or you get a black/dead console.
#   * Graphics:  the framework offers virtio-gpu only. We build the initramfs and
#                Xorg around virtio_gpu + modesetting for smooth 2D/KMS.
#   * Disk/net:  everything is virtio (virtio_blk, virtio_net, virtio_pci), so the
#                initramfs is built with those drivers forced in (no host-only).
#   * Boot:      Apple's EFI loader reads /EFI/BOOT/BOOTAA64.EFI off the ESP, so
#                we install a standalone arm64-efi GRUB there.
#
# Importing into UTM (Apple Virtualization backend):
#   1. New VM -> Virtualize -> Linux.
#   2. Leave "Boot from kernel image" UNCHECKED (we ship an EFI disk).
#   3. Skip the boot ISO; instead, after creation, replace the VM's disk with
#      this image (or use `Import` and point the drive at the .img).
#   4. Display: virtio-gpu (the default). Serial console appears as hvc0.
# See os/vm/README.md for a step-by-step and a ready-made UTM config note.
set -euo pipefail
...
mkdir -p "$ROOTFS/etc/sv/agetty-hvc0"
cat > "$ROOTFS/etc/sv/agetty-hvc0/run" <<'EOF'
#!/bin/sh
exec 2>&1
exec setsid -w agetty --noclear --autologin salt hvc0 115200 linux
EOF
chmod 0755 "$ROOTFS/etc/sv/agetty-hvc0/run"
enable_sv agetty-hvc0
...
hostonly="no"
add_drivers+=" virtio_pci virtio_blk virtio_net virtio_console virtio_gpu virtio_input virtio_mmio btrfs "
filesystems+=" btrfs "
EOF
...
KCMDLINE="root=LABEL=$ROOT_LABEL $ROOTFLAGS rootwait rw console=hvc0 console=tty0 loglevel=4 net.ifnames=0"
...
menuentry "saltOS $VERSION (Apple Virtualization / UTM, $ARCH)" {
